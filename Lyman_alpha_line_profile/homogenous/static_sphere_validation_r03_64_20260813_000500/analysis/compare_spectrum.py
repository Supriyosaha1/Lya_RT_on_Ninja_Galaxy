...
def main():
 p=argparse.ArgumentParser();p.add_argument("case",choices=("tau_1e05","tau_1e06","tau_1e07"));p.add_argument("stage",choices=("smoke","final"));a=p.parse_args();tau={"tau_1e05":1e5,"tau_1e06":1e6,"tau_1e07":1e7}[a.case];v=case_values(tau)
 ...
 fig,ax=plt.subplots();ax.step(centers,pdf,where="mid",label="RASCAS");ax.plot(centers,model,"k--",label="analytical");ax.set(xlabel="x",ylabel="unit-area p(x)");ax.legend();save(fig,out/"spectrum_linear")
 fig,ax=plt.subplots();ax.step(centers,np.maximum(pdf,1e-12),where="mid",label="RASCAS");ax.plot(centers,np.maximum(model,1e-12),"k--",label="analytical");ax.set_yscale("log");ax.set(xlabel="x",ylabel="p(x)");ax.legend();save(fig,out/"spectrum_log")
